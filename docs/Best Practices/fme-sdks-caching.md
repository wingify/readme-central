...
flowchart TD
    ...
    G[Asynchronous Refresh] --> H[Fetch current settings from Wingify in background]
...
Client SDKs fetch settings from, store them in localStorage, use them immediately, and refresh them asynchronously in the background.

Server SDKs fetch fresh settings from; or use the passed stored settings, passed while initializing the SDK.
