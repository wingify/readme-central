...
1. Check if the feature is enabled for the user:<br />`bool isEnabled = flag.isEnabled();`
2. Retrieve associated feature variables (if configured):<br />`dynamic variable = flag.getVariable("variable_key", "default_value");
...
final GetFlag? flag = await wingifyClient?.getFlag(
