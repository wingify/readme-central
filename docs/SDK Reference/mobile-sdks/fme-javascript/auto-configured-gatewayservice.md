...
2. **Faster Performance** : In the past, the SDK had to contact a gateway service to evaluate Location and UA based . With the new setup, the SDK communicates directly with VWO's DACDN (Delivery and Content Distribution Network), which handles segmentation and evaluation in real time. This significantly reduces delays and improves performance.
