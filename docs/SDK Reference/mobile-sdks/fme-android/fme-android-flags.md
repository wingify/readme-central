...
1. Check if the feature is enabled for the user:<br />`val isFlagEnabled = featureFlag?.isEnabled()`
2. Retrieve associated feature variables (if configured):<br />`val variableValue = featureFlag?.getVariable('variable_key', 'default-value') as String
