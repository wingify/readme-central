...
The `init()` method is called with the `sdkKey`and `accountId`. It initializes and returns a Wingify Client Object`vwoClient`, which can be used to perform feature  
...
When you initialize the _vwoClient_ on your mobile, it pulls the latest configurations you've done in the Wingify application.  
If/when you make any changes to the feature flags or rules within Wingify after the _vwoClient_ has been initialized on your mobile, there needs to be some way to update your _vwoClient_ with the latest settings from Wingify. This can be done via [polling](https://developers.wingify.com/v2/docs/polling).
