...
import { VWOProvider, IVWOOptions, IVWOContextModel, useGetFlag } from 'vwo-fme-react-sdk';
...
const userContext: IVWOContextModel = { id: userId };
...
2\. Pass this uuid and sessionId into the context used by `VWOProvider` (or the `useGetFlag` hook):

```tsx
const YourComponent = () => {
  // NOTE: don't annotate this object literal as `IVWOContextModel` — `useIdForWeb`
