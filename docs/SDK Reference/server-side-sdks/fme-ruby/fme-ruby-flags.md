...
Feature Flags serve as the foundation for all testing, personalization, and rollout rules within FE. To implement a feature flag, first use the _get_flag()_ method to retrieve the flag configuration. The _get_flag()_ method provides a simple way to check if a feature is enabled for a specific user and access its variables. It returns an object that contains methods like _is_enabled()_ for checking the feature's status and _get_variable()_ for retrieving any associated variable(s).
...
1. get_flag(): Fetches the feature flag configuration for the specified feature and user.
   ...
   2. get_variables() / get_variable(): Retrieves any custom variables or settings linked to the feature flag (e.g., UI colors, text, or limits).
...
| **user_context**<br />_Required_ | Hash   | Contains information about the current user, including a required unique identifier for each user. Read more about userContext [here](https://developers.wingify.com/v3/docs/fme-ruby-context).                |
...
| **default_value**<br />_Optional_ | Any    | The fallback value to return if the getVariable method encounters an error or the specified variable_key does not exist.             |
