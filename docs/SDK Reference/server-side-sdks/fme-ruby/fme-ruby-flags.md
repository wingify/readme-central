...
Feature Flags serve as the foundation for all testing, personalization, and rollout rules within FME. To implement a feature flag, first use the _get_flag()_ method to retrieve the flag configuration. The _get_flag()_ method provides a simple way to check if a feature is enabled for a specific user and access its variables. It returns an object that contains methods like _is_enabled()_ for checking the feature's status and _get_variable()_ for retrieving any associated variable(s).
...
[block:parameters]
{
  "data": {
    ...
    "1-0": "**user_context**  \n_Required_",
    "1-1": "Hash",
    "1-2": "Contains information about the current user, including a required unique identifier for each user. Read more about userContext [here](https://developers.vwo.com/v2/docs/fme-ruby-context)."
  },
  "cols": 3,
  "rows": 2,
  "align": [
