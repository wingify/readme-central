...
Feature Flags serve as the foundation for all testing, personalization, and rollout rules within FME. To implement a feature flag, first use the *get\_flag()* method to retrieve the flag configuration. The *get\_flag()* method provides a simple way to check if a feature is enabled for a specific user and access its variables. It returns an object that contains methods like *is\_enabled()* for checking the feature's status and *get\_variable()* for retrieving any associated variable(s).
...
1. get\_flag(): Fetches the feature flag configuration for the specified feature and user.
   ...
   2. get\_variables() / get\_variable(): Retrieves any custom variables or settings linked to the feature flag (e.g., UI colors, text, or limits).
...
<Table align={["left","left","left"]}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Contains information about the current user, including a required unique identifier for each user. Read more about userContext [here](https://developers.vwo.com/v2/docs/fme-ruby-context).
      </td>
    </tr>
  </tbody>
</Table>
