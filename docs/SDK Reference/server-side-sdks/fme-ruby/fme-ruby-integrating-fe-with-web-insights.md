...
| Type      | Description                                                                                                                                                                                                           |
| :-------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Integer` | The session identifier used for this flag evaluation. If `context[:sessionId]` was provided when calling `get_flag`, that value is returned. Otherwise, it defaults to the Unix timestamp at which the session began. |
...
2\. Pass this uuid and sessionId to the frontend (e.g. as view/template variables or an API response), then push them into SmartCode **before** the SmartCode script initializes.
...
2\. Pass this uuid and sessionId to your backend, and use them in the context hash when calling the FE SDK:

```ruby
context = {
  id: uuid,          # uuid received from the frontend
  sessionId: session_id, # sessionId received from the frontend
  useIdForWeb: true
}
