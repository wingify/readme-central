...
metadata:
  robots: index
---
The VWO PHP SDK includes support for **custom proxy URLs**, enabling you to route all SDK network traffic through your own proxy server. This feature provides enhanced control over request routing, offering significant benefits in environments where direct network access to VWO endpoints may be restricted or blocked.
...
1. **SDK → Proxy Server**<br />The VWO SDK sends all API and data collection requests to the proxy server, using the `proxy` specified during SDK initialization.
