...
WingifyUserContext userContext = new WingifyUserContext();
...
GetFlag flag = wingifyClient.getFlag("feature_key", userContext);

# Track a metric conversion for the specified event-name
wingifyClient.trackEvent("event_name", context);

# Send a user attribute to Winigfy
wingifyClient.setAttribute("attribute_key", "attribute_value", context);
```
