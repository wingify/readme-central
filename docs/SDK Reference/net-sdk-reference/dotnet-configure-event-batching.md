...
VWO SDKs provide two ways to configure when events should be sent to Wingify servers. This can be done only while instantiating the SDK by passing ***batchData*** key along with one or both of the following keys.
...
VWO SDKs also provide a functionality to call a function upon successful/failure batch network request to Wingify server. You can pass ***FlushCallback*** in *batchData* config at the time of launching the SDK.
