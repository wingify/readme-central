...
Since Wingify SDKs queue the events and are sent in a network request only upon meeting the configurational options limit passed in **batchEvents** config, there might be cases when you would need to manually flush the queue so that data could be sync with Wingify server to be reflected in the respective Campaigns' reports.
...
> 🚧 JavaScript SDK
>
> The port of Node.js SDK i.e. JavaScript SDK, which runs on the client-side does not support event-batching as everything happens in the context of a single-user and the events are not much. The browsers also handle multiple calls gracefully.
