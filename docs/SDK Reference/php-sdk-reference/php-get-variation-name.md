...
It takes the same parameters and returns the same value as [Activate API](https://developers.wingify.com/docs/php-activate). The only difference is that this API method does ***not*** send a tracking impression to the Wingify server. This API method is used to get the variation assigned to the *userId*.\
The behaviour of the two API methods, that is, [activate](https://developers.wingify.com/docs/php-activate) and [getVariationName](https://developers.wingify.com/docs/php-get-variation-name) is identical otherwise.
...
If [User Storage Service](https://developers.wingify.com/docs/php-implement-a-user-storage-service) is provided, campaign activation is mandatory before tracking any goal, getting a variation of a campaign, and getting the value of the feature's variable.
