...
It takes the same parameters and returns the same value as [Activate API](https://developers.wingify.com/docs/python-activate). The only difference is that this API method does ***not*** send a tracking impression to the Wingify server. This API method is used to get the variation assigned to the *userId*.\
The behaviour of the two API methods, that is, [activate](https://developers.wingify.com/docs/python-activate) and [getVariationName](https://developers.wingify.com/docs/python-get-variation-name) is identical otherwise.
