...
With different seed values for both rules, it is possible that the same user gets bucketed into `variation 1` in the first rule, and `variation 2` in the second rule. This is desirable for most usecases, where customers want the same user to see different experiences, in different rules.

But if the salt is explicitly set for a rule, then the bucketing seed is determined by ***`seed = salt_userId`***

Now, for the same example as above, if the salt is set to `xyz`, for both rules the seed is the same, `xyz_abc123`. Which means, that the user will get bucketed into the same variation in both rules.

<br />

## Structural Limitations

1. Bucketing into variations is applicable only when variations are present for the rule, so this is **not** applicable for **Rollouts**
