...
4. Click Save
5. Copy/note the API key that Wingify generates. You'll need this to configure Mixpanel.

## **Step 2 - Configure Wingify as a Destination in Mixpanel:**

* In Mixpanel, navigate to Integrations.
* Click Add a new integration, select Wingify.
...
After configuring pre-segmentation in the Wingify app, ensure your SDK passes the appropriate user context for evaluation. Let’s see an example of using Mixpanel cohorts in feature evaluation.
...
* Pass this context to `getFlag()` so Wingify can evaluate the feature flag using Mixpanel cohort pre-segmentation.
...
* The key "vwo_user_id" must exactly match the identifier defined in your Wingify pre-segmentation rule.
