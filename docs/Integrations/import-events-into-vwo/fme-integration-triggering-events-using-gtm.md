...
The **Wingify Event** is a custom Google Tag Manager (GTM) template designed to simplify the process of sending custom events and properties to Wingify, instead of writing custom JavaScript code to push data to the window.Wingify object, this feature enables users to configure event tracking directly within the GTM interface.
