...
**Segment** is a leading Customer Data Platform (CDP) that collects, cleans, and routes your customer data to hundreds of tools. By integrating Segment with **Wingify Feature Experimentation**, you can leverage Segment’s powerful "Personas" and "Audiences" to drive your experimentation strategy.

## **What This Integration Achieves**

This integration allows you to use Segment-computed traits and audiences for Wingify feature flag targeting. By importing these segments into, you can perform highly granular rollouts and A/B tests based on real-time user data captured across your entire tech stack.
...
* **Audience-driven A/B testing:** Combine Segment’s unified customer data with’s experimentation engine.
* **Data-driven decisions:** Leverage Segment’s rich cross-platform user data for more effective feature rollouts.

## **Step 1: Enabling the-Segment Integration for your Wingify Account**

To enable the Segment integration in Wingify:
...
## **Step 2: Configure Wingify Cloud Mode (Actions) in Segment**

Before Wingify can see your audiences, you must connect them via the Cloud Mode destination:
...
3. Select the **Wingify Cloud Mode (Actions)** destination.  
   **INFO:** If you can’t find the **Wingify Cloud Mode (Actions)** destination, you need to install it by performing the steps mentioned [here](https://segment.com/docs/connections/destinations/catalog/actions-vwo-cloud/).  
   Enabling the Wingify Cloud Mode (Actions) destination is essential as it is the channel through which the events, attributes, and audiences are transited to Wingify from Segment.
...
5. Click **Save**.
   ...
   **Note:** Segment audiences populate in Wingify only after your website receives a visitor following this connection.

## **Step 3: Import & Activate Segment Audiences in**

1. Back in Wingify (**Configurations** > **Integrations** > **Segment** > **Config**), click **Add Audience**.
...
**Note: Syncing:** Wingify takes ~24 hours for the initial sync, then updates every 24 hours. Use **Sync all** or the vertical ellipsis (⋮) for a manual sync.

## **Step 4: SDK Setup (Node.js Example)**

#### **4.1 Install and Initialize the Wingify SDK**
...
**Reference:** [Wingify Gateway Service Doc](https://help.vwo.com/hc/en-us/articles/16147461611161-Integrating-VWO-With-Segment)
...
## **Step 5: Setting Up Pre-Segmentation in**
...
let context = {
    ...
    customVariables: {
        // "PayPal" must match the key defined in Wingify Pre-segmentation
        "PayPal": "650412318e2-46d5-cfb07a69958"
    }
};
const featureFlag = wingifyClient.getFlag('featureKey', context);
...
* **Wingify Cloud Mode (Actions):** This is the essential channel for transiting audience data from Segment to.
...
* **24-Hour Sync:** Be mindful that Wingify syncs Segment audience data every 24 hours by default. Use **Manual Sync** for immediate testing.
* **ID Mapping:** Ensure the key used in `customVariables` matches the identifier name defined in's rule configuration.
